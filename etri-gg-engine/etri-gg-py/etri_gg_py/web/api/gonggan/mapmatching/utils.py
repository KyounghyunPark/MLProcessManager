...
class Utils:
    ...
    def matching_result2(self, link_id, road_name, speed_limit, distance):
        return (link_id, road_name, speed_limit, distance)

    # 여기서부터 아래 3개 함수는 map_matcher(OOSP).py findNearestLink 함수에서 사용되는 함수들
    def get_distance_point_to_line_in_m(
        self,
        p,
        line_start,
        line_end,
    ):  # 데이터 넣을 떄는 start,end, car 순으로 넣음
        # 선분과 점 사이의 거리 계산
...
def main():
    ...
    start_node_df = raw_node_df.rename(
        columns={"NODE_ID": "F_NODE", "geometry": "F_NODE_geometry"},
    )[["F_NODE", "F_NODE_geometry"]]
    end_node_df = raw_node_df.rename(
        columns={"NODE_ID": "T_NODE", "geometry": "T_NODE_geometry"},
    )[["T_NODE", "T_NODE_geometry"]]
    ...
    # geometry 결합
    link_node_join_df["geometry"] = (
        link_node_join_df["F_NODE_geometry"]
        + link_node_join_df["geometry"]
        + link_node_join_df["T_NODE_geometry"]
    )

    # 라인 문자열을 배열로 변환
    link_node_join_df["geometry"] = link_node_join_df["geometry"].apply(
        Utils.line_str_to_mesh_str,
    )
